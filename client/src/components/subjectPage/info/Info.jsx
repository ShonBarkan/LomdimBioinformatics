const Info = ({ info = [], titleFontSize = 20 }) => {
  // Fallback if info isn't an array
    ...
    <div className="space-y-6">
      {info.map((item, index) => (
        <div 
          ...
          className={
            isTopLevel
              ? 'border-b border-gray-200 pb-6 last:border-b-0 last:pb-0'
              : 'pl-4 md:pl-6 border-r-2 border-indigo-200 pr-4 md:pr-6'
          }
        >
          {/* Title */}
          <h3 
          ...
          {/* Sub Info (Recursive) */}
          {Array.isArray(item.subInfo) && item.subInfo.length > 0 && (
            <div className="mt-4">
